...
class ExampleUtils
{
    ...
    {
        $dataFileDate = self::getDataFileDate($engine);

        return strtotime('today') > $dataFileDate + mktime(0, 0, 0, 0, self::DATA_FILE_AGE_WARNING, 0);
    }
    ...
    public static function checkDataFile($engine, $logger)
    {
        if (!empty($engine)) {
            ...
            $logger->log(
                'info',
                "Using a '" . self::getDataFileTier($engine) . "' data file created " .
                "'" . date('d/m/Y', $dataFileDate) . "' from location " .
                "'" . $engine->engine->getDataFilePath() . "'"
            );

            if (self::dataFileIsOld($engine)) {
                $logger->log(
                    'warn',
                    'This example is using a data file ' .
                    "that is more than '" . self::DATA_FILE_AGE_WARNING . "' days " .
            ...
            if (self::getDataFileTier($engine) === 'Lite') {
                $logger->log(
                    'warn',
